...
export const FileUpload = ({ onChange }: { onChange?: (files: File[]) => void }) => {
  ...
  return (
    <div {...getRootProps()} className="w-full">
      <motion.div
        onClick={handleClick}
        className="p-10 block rounded-2xl cursor-pointer w-full relative overflow-hidden border border-white/15 hover:border-white transition-colors"
      >
        <input
          ref={fileInputRef}
          ...
          <div className="relative w-full mt-10 max-w-xl mx-auto">
            ...
            {!files.length && (
              <motion.div
                ...
                className="relative z-40 flex items-center justify-center h-32 w-full max-w-[8rem] mx-auto rounded-xl bg-white/10 dark:bg-black/30 backdrop-blur-xl border border-white/20 hover:border-white transition-colors"
              >
                {isDragActive ? (
